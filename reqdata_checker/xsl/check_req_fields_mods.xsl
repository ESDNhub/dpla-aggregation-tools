...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        ...
        <xsl:for-each select="//oai:record[./oai:header[not(@status='deleted')]]">
            ...
            <xsl:if test="not($title) or not($coverage) or not($rights) or not($thumburl) or not($refurl) or not($owninginst) or not($language) or not($type)">
                <tr>
                    ...
                    <td>
                        ...
                        <xsl:if test="not($refurl)">
                            <p>Reference URL</p> 
                        </xsl:if>
                        <xsl:if test="not($owninginst)">
                            <p>Owning Institution</p>
                        </xsl:if>
                    </td>
                    <td>
